...
func (c *Client) ListPokemonsInformation(param string) (PokemonInformation, error) {
	if strings.Trim(param, " ") == "" {
		fmt.Println("Please specify a location")
		return PokemonInformation{}, fmt.Errorf("Not a valid location")
	}

	endpoint := "https://pokeapi.co/api/v2/pokemon/" + param
	...
	if ok {
		json.Unmarshal(data, &pokemonInformation)
	...
	if resp.StatusCode > 399 {
		return PokemonInformation{}, fmt.Errorf("Bad request: %v", resp.StatusCode)
	}
